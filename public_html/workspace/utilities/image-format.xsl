...
<xsl:template name="img">
	...
	<xsl:param name="default-mode" select="'normal'"/>
...
<xsl:template name="image-path">
    ...
    <xsl:variable name="filename">
    	<xsl:choose>
    		...
    		<xsl:when test="$default-mode = 'hotel'">
    			<xsl:value-of select="concat($settings/default-hotel-image/item/image/@path,'/',$settings/default-hotel-image/item/image/filename)"/>
    		</xsl:when>
    		<xsl:when test="$default-mode = 'destination'">
    			<xsl:value-of select="concat($settings/default-destination-image/item/image//@path,'/',$settings/default-destination-image/item/image/filename)"/>
    		</xsl:when>
    		<xsl:otherwise>
    			<xsl:value-of select="concat($settings/default-background-image/item/image//@path,'/',$settings/default-background-image/item/image/filename)"/>
    		</xsl:otherwise>
    	</xsl:choose>
    </xsl:variable>
	...
	<xsl:choose>
		<xsl:when test="$real-mode='resize'">
			<xsl:value-of select="concat( $root , '/image/1/' , $width , '/' , $height , '/0/' , $filename)" />	
		</xsl:when>
		
		<xsl:when test="$real-mode='crop'">
			<xsl:value-of select="concat( $root , '/image/4/' , $crop/@width ,'/', $crop/@height ,'/', $crop/@x1 ,'/', $crop/@y1 ,'/', $width ,'/', $height ,'/0/',$filename)" />	
		</xsl:when>
		
		<xsl:when test="$real-mode='crop-fill'">
			<xsl:value-of select="concat( $root , '/image/2/' , $width ,'/', $height ,'/5/0/',$filename)" />	
		</xsl:when>
		
		<xsl:when test="$real-mode='resize-canvas'">
			<xsl:value-of select="concat( $root , '/image/3/' , $width ,'/', $height ,'/5/fff/0/', $filename)" />	
		</xsl:when>
		
		<xsl:otherwise>
			<xsl:value-of select="concat($root , '/image/0/' , $filename )" />
		</xsl:otherwise>		
	</xsl:choose>
