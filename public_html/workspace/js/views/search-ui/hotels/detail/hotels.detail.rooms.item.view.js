...
/*			if(roomdetails.SelectedRoom.maxExtraChildren == 0){
				console.log('higfg');
				}
*/			return _.extend({hotelSelected: this._hotelSelected,viewPosition: this._viewPosition, cost: cost}, this.model.toJSON());
		...
		*/
		handleRoomsUpgrade:function(Rooms){
			  ...
			  //if(this.model.collection.models[Position - 1].maxExtraChildren == Rooms.Upgraderoom.maxExtraChildren){
		...
		handleChangeRoom: function(ev){
			...
			var roomdetails = new RoomSelectorView({ model: _.extend({ 
																	 RoomPosition: Position, 
																	 SelectedRoom: this.model.collection.models[Position - 1].attributes, 
																	 "rooms": reqres.request('search:get:hotel:selected').get('rooms').toJSON()
																	 }) 
												   });
			//console.log(roomdetails.SelectedRoom.maxExtraChildren);
			roomdetails.render();
		},
	});
